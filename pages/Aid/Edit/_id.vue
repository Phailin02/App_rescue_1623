...
export default {
  ...
  methods: {
    ...
    save() {
      if (this.create.title && this.create.describe) {
        this.saveData()
      } else {
        Swal.fire({
          position: 'center',
    ...
    async saveData() {
      ...
      try {
        ...
        console.log(config)
        await axios.put('/firstaid/update', formData, config)
      ...
      .then((res) => {
        ...
        for (const data of this.responseData) {
          ...
          if (data.id === Number(this.$route.params.id)) {
            this.create.title = data.first_aid_title
            this.create.describe = data.description
