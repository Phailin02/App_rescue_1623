<template>
  <div>
    ...
    <div>
      <v-card>
        <v-card-title>
          <strong>ຈັດການຂໍ້ມູນໂຮງໝໍ</strong>
          <v-spacer></v-spacer>
        </v-card-title>
        <v-data-table :headers="headers" :items="responseData" :search="search">
          ...
          <template v-slot:item.options="{ item }">
            <div>
              ...
              >
              <Nuxt-link :to="`/Hospital/Edit/${item.id}`">
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'ຊື່ໂຮງໝໍ', value: 'hospital_name' },
  ...
  methods: {
    ...
    async deleteItem(itemId) {
      try {
        ...
        // Use the axios instance for making the request
        await axios.delete('/hospital/delete', config)
  ...
  async mounted() {
    try {
      const response = await axios.get('/hospital/view?limit=&page=')
